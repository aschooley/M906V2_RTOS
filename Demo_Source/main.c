...
void main( void )
{
	...
	//{
		...
		xTaskCreate( Test1Task, "Reg1", configMINIMAL_STACK_SIZE, NULL, 0, NULL );
		xTaskCreate( Test2Task, "Reg2", configMINIMAL_STACK_SIZE, NULL, 0, NULL );
...
xQueueMessage xMessage;
	...
	for( ;; )
	{
		...
		ucState = 0;//( halButtonsPressed() & BUTTON_UP );
...
static void prvSetupHardware( void )
{
	...
	//		P1DIR |= BIT0;
		...
		TA0CCTL0 = CCIE;
		// Set the capture compare to zero, essentially turns the timer off.
		TA0CCR0 = 0;
		// SMCLK, UP mode, prescale /8
		TA0CTL = TASSEL__SMCLK | MC__UP | ID__8;
		//TA0CTL = TASSEL__SMCLK | MC__CONTINUOUS | ID__8;
#endif
		// interrupts enabled
...
void vApplicationSetupTimerInterrupt( void )
{
	...
	const unsigned short usACLK_Frequency_Hz = 10000;
//AJS const unsigned short usACLK_Frequency_Hz = 32768;
